...
class PfsFile:
    ...
    def parse(self, blob: bytes) -> None:
        ...
        while offset < len(blob) - struct.calcsize(PfsFile.PFS_FOOTER):
            ...
            # parse the section
            nt2 = namedtuple('PfsHeaderSection',
                             ['guid', 'hdr_ver', 'ver_type',
                              'version', 'reserved', 'data_sz', 'data_sig_sz',
                              'metadata_sz', 'metadata_sig_sz'])
            try:
                pfs_sect = nt2._make(struct.unpack_from(PfsFile.PFS_SECTION, blob, offset))
            except struct.error as e:
                raise RuntimeError from e
            ...
            if shard.blob and shard.guid == 'fd041960-0dc8-4b9f-8225-bba9e37c71e0':
                self._parse_info(shard.blob)
            elif shard.blob and shard.guid == '233ae3fb-da68-4fd4-92cb-a6229a611d6f':
...
class Plugin(PluginBase):
    ...
    def _get_shards_for_blob(self, blob: bytes) -> List[ComponentShard]:
    ...
    def _run_uefi_extract_on_md(self, test: Test, md: Component):
