...
def _sigul_detached_sign_data(contents: bytes, config: str, key: str) -> str:
    ...
    if ps.wait() != 0:
        if not ps.stderr:
            raise PluginError('Failed to sign')
        raise PluginError('Failed to sign: %s' % ps.stderr.read().decode())
...
class Plugin(PluginBase):
    def __init__(self):
        PluginBase.__init__(self, 'sign-sigul')
